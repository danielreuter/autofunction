/**
 *
 * Tests that the parser outputs exactly the same
 * results as the usual `zod-to-json-schema` package
 * when the input is a conventional Zod schema.
 */
...
export const allParsersSchema = z
  .object({
    ...
    objectStrip: z
      .object({ foo: z.string(), bar: z.number().optional() })
      .strip(),
    // promise: z.promise(z.string()), changed this
...
describe("mod <-> zod-to-json-schema", () => {
  it("should output the same schema", () => {
    const schema1: any = modToJsonSchema(allParsersSchema);
    const schema2 = zodToJsonSchema(allParsersSchema);
